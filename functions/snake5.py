...
# Save timeline of button presses and apple eaten
# 1: Left Arrow Key,	2: Right Arrow Key, 	8: Apple Eaten
start_time = 0
timeline = []
...
def start(event):
	'''start the game'''
	global flag, timeout, start_time
	if flag == 2:
		flag = 1
		fen.bind('<Left>', tleft)
...
def pommeQ():
	...
	global x, y, pomme, points, points_per_apple, timeline, start_time
	...
	for i in range(0,len(pomme)):
		if ([True]==[x==p[i][0] and y==p[i][1] for p in [pomme]]):
			...
			timeline.append( [current_time, 8] )
			apple()
			break
		else:
			if i==0:
...
def tright(event):
	global dx, dy, nButtonsPressed, timeline, start_time
	# Update stats
	nButtonsPressed += 1
	current_time = time.clock() - start_time
	timeline.append( [current_time, 2] )
...
def tleft(event):
	...
	nButtonsPressed += 1
	current_time = time.clock() - start_time
	timeline.append( [current_time, 1] )
